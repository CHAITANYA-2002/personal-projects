...
1. [`notebook/01_data_cleaning_feature_engineering.ipynb`](notebook/01_data_cleaning_feature_engineering.ipynb) — profiles the raw tables, audits missing values and referential integrity, and engineers three analysis-ready tables (`f1_result_features`, `f1_driver_features`, `f1_season_driver`) back into the database.
2. [`notebook/02_f1_eda.ipynb`](notebook/02_f1_eda.ipynb) — the exploratory data analysis: themed sections (snapshots, geography, demographics, constructors, reliability, era trends, the champion thesis, and a correlation overview) rendered as ~15 seaborn charts from live SQL.

Both notebooks read the connection from the `F1_DB_*` environment variables above, falling back to `root`/`root`/`localhost` if they are unset, so no password is committed in source. Notebook 2 depends on the tables built by Notebook 1, so run them in sequence.

### Run the dashboard (Streamlit UI)

An interactive dashboard over the same database lives in [`app.py`](app.py):
...
It opens in the browser with six pages:

- **Overview** — headline KPIs, races-per-decade, all-time win leaders, and the ERD.
- **Drivers** — filter by minimum starts, browse the `f1_driver_features` table, and drill into any driver's season-by-season points.
- **Constructors** — race wins and constructors' championships by team.
- **Race Explorer** — pick any season and race to see the full classified result with the podium.
- **The Champion Thesis** — the five thesis findings as live charts (consistency, reliability, car-vs-driver, age curve, qualifying correlation).
- **SQL Query Runner** — run the project's showcase queries live, or type your own; a guard restricts input to read-only `SELECT` / `WITH` / `SHOW` / `EXPLAIN`.

The dashboard reads the same `F1_DB_*` environment variables (fallback `root`/`root`/`localhost`). The Drivers and Champion Thesis pages need the feature tables from Notebook 1; the app shows a warning and stays usable if they are missing.

## Output folders

- `results/*.txt` contains captured MySQL output from a recorded run.
- `results/screenshots/` contains six standalone presentation charts referenced by `docs/findings_report.md`.
- `docs/findings_report.md` gives a compact written interpretation of the thesis results.
- `notebook/01_data_cleaning_feature_engineering.ipynb` and `notebook/02_f1_eda.ipynb` are the interactive cleaning + EDA walkthrough (charts embedded inline).
...
6. Run [`notebook/01_data_cleaning_feature_engineering.ipynb`](notebook/01_data_cleaning_feature_engineering.ipynb) then [`notebook/02_f1_eda.ipynb`](notebook/02_f1_eda.ipynb) to see the cleaning pipeline and the evidence presented as charts.
7. Launch [`app.py`](app.py) to explore the same evidence interactively.

This sequence lets a reviewer evaluate the project at four levels: database correctness, SQL technique, analytical communication, and interactive delivery.

## Credits, data sources and trademarks

Full attribution for every external asset is in [`CREDITS.md`](CREDITS.md). In short:
...
- **Circuit layouts** — real track geometry from [`bacinger/f1-circuits`](https://github.com/bacinger/f1-circuits), MIT licensed.
- **Driver photographs** — English Wikipedia lead images; individual licences vary (mostly Creative Commons or public domain). Where no photograph exists, the interface shows a team-coloured monogram — never a synthetic or stand-in face.
- **Constructor logos** — team marks shown in the dashboard are **registered trademarks of their respective owners**. They are used nominatively, solely to identify the team whose results are displayed in this non-commercial educational project, and imply no endorsement, affiliation or sponsorship. Teams without an available mark fall back to a generated monogram.
